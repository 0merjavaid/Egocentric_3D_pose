...
train_loader = torch.utils.data.DataLoader(ds.batched(6), num_workers=4, batch_size=None)
...
valid_loader = torch.utils.data.DataLoader(ds_valid, num_workers=4, batch_size=6)
...
model.to(device)
logging.basicConfig(filename="./train_logs/resattnet_part2.log", format='%(message)s',
...
#             epoch_acc = running_corrects.double() / dataset_sizes[phase]
        ...
        print()
        torch.save(model.state_dict(), f"./ckpts/resattnet/{epoch}_epoch_{epoch_loss}.pth")
...
model.load_state_dict(torch.load("./ckpts/resattnet/7_epoch_0.0007464294610069335.pth"))
