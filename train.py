...
print(len(train_url))
print(len(test_url))
ds = wds.WebDataset(train_url).shuffle(1000, initial=1000).map(decode_train, handler=ignore_and_continue).to_tuple("image", "pose", "heatmaps", handler=ignore_and_continue)
...
model.load_state_dict(torch.load("./33000th iteration.pth"))

best = train_model(model, pose_criterion, hmp_loss, hmp_loss2, l1_loss, cos_sim, optimizer_ft, 7)